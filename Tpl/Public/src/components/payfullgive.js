...
class payfullForm extends React.Component {
  constructor(props) {
        super(props)
        this.state = {
        		createing:false,
  ...
  handleSubmit = (e) => {
    ...
    this.props.form.validateFields({},(err, values) => {
      if (!err) {
        ...
        var formData={
          act_obj:'所有在支付宝口碑消费超过过'+values.min_cost+'元的人群',
          name:values.actName,
        ...
        $.ajax({
          type:'POST',
          dataType:'json',
          url:AJAX_URL+'/Coupon/ajaxCreateMinus'+token,
          data:formData,
 ...
 onShopOk=(x)=> {
  ...
  render() {
    ...
    return (
      ...
      <p className="ctitle" style={{paddingLeft:'30px',marginBottom:20}}>场景营销-满返券 活动设置</p>
      ...
      <div className="giveFormDiv" >
        ...
        >
        支付宝支付满
          <span className="ant-form-text" style={{padding:'0px 10px'}}>
          

          	{getFieldDecorator('min_cost',{
        		initialValue:100,
            	rules:[
                	{ required: this.state.coinShow=='none'?true:false,type:'number',message:"请输入正确的消费门槛",whitespace:true,pattern:/^(?!0+(?:\.0+)?$)(?:[1-9]\d*|0)(?:\.\d{1,2})?$/}
                ]
          })(
            <InputNumber min={1} style={{width:60}} onChange={this.payNeedChange} placeholder=""/>
          )}
        ...
        <Row gutter={16} style={{display:this.state.moreSet}}>
            ...
            <FormItem
              ...
              >
                    ...
                    <Col span="5" className="ant-form-splitCol">
...
payfullForm = Form.create()(payfullForm);


export default payfullForm;
